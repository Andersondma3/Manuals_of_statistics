...
## Gráfico para el comportamiento de la respuesta entre lotes

El código general de este gráfico es `qplot(X,Y,geom="boxplot")`, *a continuación se le da un ejemplo*

```{r, warning=FALSE, message=FALSE}
load("colorantes.Rdata")
library(ggplot2)
qplot(lote,prod,geom="boxplot",ylab="producción",data=base)
```
*Intepretación del gráfico:* Hay algunos lotes que tienen producciones muy bajas mientras que otros las tienen muy altas, aunque no interesa identificar cuáles son los lotes con producción más baja o más alta puesto que los lotes son aleatorios.
...
# Diferencias entre los diferentes tipos de modelos
...
## Sobre los Modelos lineales mixtos
...
# Supuestos en los modelos mixtos


Los modelos mixtos permiten manejar estructuras de variabilidad que no son homogéneas y homogénas (homocedasticidad), este último no es tan común en modelos mixtos. Con respecto al análisis de varianza (ANOVA) sabemos que este no necesita linealidad. Por otro lado sabemos que esto es distinto cuando el modelo contiene covariabales ó variables numéricas como en regresión, en el caso de los modelos mixtos necesitamos linealidad. Si sucede el caso no se cumple, debemos realizar una transformación. 
...
# Leyenda
legend("topright", 
       legend=c("Media Global (mu)", "Media Grupal (mu_i)", "Efecto aleatorio"),
...
+ Interesa determinar si hay efecto de las unidades del segundo nivel, pero no interesa estimar la magnitud de los mismos,
ya que son solo una muestra de una población mayor de unidades infinitas.
...
Esto nos dice que porcentaje de varibilidad se debe a las unidades del nivel 2. Si hay **baja** variabilidad intra clase $\rho_I \to 0$, quiere decir que las unidades del segundo nivel son muy similares, por el contrario si $\rho_I \to 1$, nos esta diciendo que son muy diferentes.
...
El efecto del i-ésimo del factor del nivel 2 se denota con: $\tau_i$  

El efecto aleatorio de la j-ésima unidad de nivel 2 se denota con: $\delta_j$ con su distribución $\delta_j\sim\mathcal{N}(0,\sigma_\delta)$

El efecto de interacción para el i-ésimo factor en la  j-ésima unidad de nivel 2 se denota con: $(\tau\delta)_{ij}$ 
...
Tenenos dos varianzas: $\sigma_{\varepsilon}$ varianza del error, la cual corresponde a la de muestra a muestra dentro de un día y una observación. Por otro lado $\sigma_{\delta}$ varianza de los efectos aleatorios del día.
...
*Nota:* Recuerede que en un modelo com imteracción podemos hacer análsis de manera marginal.
...
Definición de la verosimilitud creada en la clase: *La verosimilitud es que tan creíble es la estimación de un parámetro, basado en los datos de la muestra. Con el objetivo de dar una medida de la credibilidad de la estimación.*
...
**Se empieza estimando $\sigma^2_{epsilon}$ con el CMRes y se sustituye en la primera ecuación:**
...
Con esta diferencia podemos ver potencia,el ancho del intervalo. Para ello se tiene que simular varias veces y guardar esos resultados, para analizar cada uno de los casos anteriores. Para al final sacar el promedio del ancho del intervalo. De manera que cambiando `r` y `n` se llegue a un ancho del intervalo deseado. *Se encuentra una simulación de esto en la última sección*.
...
$$H_0:\alpha_i = 0  \space \forall_i \text{ ó } \space \alpha_2 = \alpha_3 = \alpha_3$$
...
### Intervalos de confianza cuando hay interacción 


El `p-value` se compara contra $\alpha$, ya que la función hace la correción de una vez.

```{r, eval=FALSE}
em.mod1=emmeans(mod1,pairwise~maq|dia,adjust="bonferroni")
#emmeans(mod1,pairwise~factor_fijo|otro_factor,adjust="bonferroni")
#factor_fijo|otro_factor son las comparaciones considerando el otro factor
```
...
### Intervalos de confianza para diferencias de factores fijos sin interacción 

```{r, eval=FALSE}
em.mod1=emmeans(mod1,pairwise~maq,adjust="tuckey")
#Intervalo par diferencias entre máquinas
```
...
Podemos mejorar los intervalos, debido a que la función `emmeans`, toma en cuenta todas las diferencias posibles. Sin embargo, no todas las diferencias son significativas, por lo que se necesita aplicar el siguiente proceso para mejor su precisión.
...
Cuando el diseño es de dos niveles con un factor en cada nivel, al mismo se le puede llamar como parcelas divididas. Un ejemplo de esto sería un experimento que quiere comparar el crecimiento de dos especies de papa en dos tipos de clima. En donde se siembran dos especies de papas en fincas que se encuentran en un clima frío y en otras que tienen un clima caliente.
...
```
A la izquierda podemos ver los resultados del experimento, sin embargo podemos ver que la variable temperatura se encuentra como una categoría. Lo que se nos viene la siguiente pregunta: *¿Podemos convetirla en una variable numérica?*, por lo que tratamos de unir todas las respuesta, como lo podemos observar en el gráfico de la derecha, esto nos puede ocasionar un problema, ya que al realizar esto, estaríamos ignorando el cumplimiento del supuesto de linealidad, debido a que este no se cumple, no podemos convertir la variable *Temperatura* en una variable numérica. 
...
Para realizar ese proceso tome como ejemplo, los coeficientes del siguiente modelo:


```{r}
round(summary(mod1)$coef[,1],2)
```

*Nota:* Recuerede que este análisis es marginal, solo para las temperaturas. Además utilice la función `contrasts(variable)`, para guiarse al momento de construir los vectores.
...
\text{Nivel 2 (Instructor)} & \to \text{Anidado en Escuela} \\
...
### Gráfico para representar gráficamente los datos y observar variabilidad
...
*Nota:* El `layout` normalmente se pone en lugar del 3, la cantidad de columnas que se quieren ver.
...
La cantidad de niveles se debe analizar dentro de cada factor, en el ejemplo anterior se debe analizar dentro de cada región, por lo que en este caso hay 2 por región.

*Observe el siguiente gráfico, para observar las diferencias entre escuelas*
...
*En un país extremadamente grande habián muchas escuelas de intructores y se selccionan 3 escuelas. Además se quiere generalizar el estudio para todas las escuelas, por lo que me interesa es observar la variabilidad entre estas. Dentro de cada escuela, hay una cantidad considerable de intructores, así que se seleccionaron en cada una de las escuelas elegidas anteriormente dos intructores, por lo que estos ya son una muestra del total de instructores y cada uno de esos dos intructores tiene dos grupos, los cuales son los que voy a observar para el estudio.*
...
Como son independientes las comparaciones, no se necesita realizar correción, ya que lo que nos interesa es si existen diferencias entre ellos, pero dentro de cada grupo. Lo cual nos lleva a la siguiente hipótesis:
...
Note que en los `gl` dan (n-1), ya que el total de obervaciones eran 12 y sumando la columna de los `df` nos da como resultado 11. `instructor1` es la variable no anidada.Por lo tanto, lo correcto es que se haga con la variable normal.
...
sim_ancho <- function(n, r, k = 10, med = 1500, sd.lote = 42, sd.res = 49.51) {
  ...
  res <- matrix(nrow = k, ncol = 3) # Creación de la matr´z de resultados 
  ...
  for(i in 1:k) {  
    ...
    prom = apply(res, 2, mean)# Se cálcula la media de los resultados de la matriz
  }
...
## Combinaciones múltiples del ancho del intervalo 
...
## Simulación de potencia para diferentes combinaciones de días y máquinas
...
## Simulación de potencia, para un experimento de tres niveles, sin interacción

La simulación se basa en el experimento de papel visto en clase

```{r, eval=FALSE}
library(lme4)
nsim <- 800 #número de simulaciones
...
n_met <- 3
n_temp <- 4
#Estos se obtienen del VarCorr(mod)
sd.lote <- 1.7 
sd.res <- 2.3619
#Estos se obtienen del fixef(mod)
mu <- 30.4722
...
# Se creán los vectores para la simulación
sdlote =c()
sdres = c()
...
cat("El promedio de la varianza del lote es:", round(mean_lote,3), "\nEl promedio de la distribucion de sigmas cuadrados es", round(mean_res,3), "\nLa varianza del lote fijada es" , sd.lote^2, "\nEl sigma fijado es de:", sd.res^2)
```


## Simulación de base de datos de pinturas

El objetivo de est simulación es generar una nueva base de datos, para 4 días con 3 tarros por día

```{r, echo=FALSE}
load("pinturas1.Rdata")
base$aplicacion = factor(base$aplicacion)
base$tarro = factor(base$tarro)
base$dia = factor(base$dia)
```

**Primero debemos crear el modelo originial**

```{r}
mod1 = lmer(reflect~aplicacion+(1|tarro)+(1|dia), data = base)
VarCorr(mod1)
```


```{r}
med=tapply(base$reflect,base$aplicacion,mean) 
med # medias de cada aplicación
```

**Procedemos a realizar la simulación**
...
dia=factor(rep(c("d1","d2","d3","d4"),each=9)) 
# Creamos la variable tarro 
tarro = factor(rep(1:12,each = 3)) # 3 tarros por 4 días
#Creamos la variable apliación
aplicacion=factor(rep(1:3,12)) # A todos los tarros se les hiciern todas las aplicaciones
#Simulamos el efecto del tarro
ef_tarro = rnorm(12,0,sd_tarro)[tarro]
#Simulamos el efecto del día
ef_dia = rnorm(4,0,sd_dia)[dia]
#Asignación de cada aplicación
muj = med[aplicacion]
#Simulamos el error
epsilon=rnorm(36,0,sd_res)
#Simulación de respuesta final
reflect1 = muj + ef_tarro+ef_dia+epsilon
```

**Respuesta observada**

```{r}
dotplot(reflect~aplicacion|dia,groups=tarro,type=c("p","a"),base,ylim=c(55,95))
```
**Respuesta Similar**


```{r}
dotplot(reflect1~aplicacion|dia,groups=tarro,type=c("p","a"),ylim=c(55,95)) 
```
