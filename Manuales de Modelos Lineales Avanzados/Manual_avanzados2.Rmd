...
$$y_{iTjkl} = \beta_0+\alpha_i+\gamma_1T+\omega_iT+\delta_{k(i)}+\lambda_{k(i)}T+\varepsilon_{ikl}$$
...
+ $\gamma_1:$ pendiente del tiempo 

+ $\omega_i:$ Efecto de interacción entre el i-ésimo método y la temperatura
...
*A continuación se le presenta este modelo en r*

```{r,eval=FALSE}
mod=lmer(res~1+metodo*temp+(1+temp|lote),data=base)
```
...
ggplot(df_lineas, aes(x = temp, y = res, color = metodo)) +
  ...
  # --- SOLUCIÓN: Usar geom_segment directo para las flechas ---
  # Alpha 1 (Método 1 vs Media Global)
  geom_segment(aes(x = 200, xend = 200, y = 28, yend = 32), 
               arrow = arrow(ends = "both", length = unit(0.15, "cm")), 
               color = "#377EB8", inherit.aes = FALSE) +
  annotate("text", x = 203, y = 30, label = "alpha[1]", parse = TRUE, color = "#377EB8") +
  
  # Alpha 3 (Método 3 vs Media Global)
  geom_segment(aes(x = 198, xend = 198, y = 28, yend = 36), 
               arrow = arrow(ends = "both", length = unit(0.15, "cm")), 
               color = "darkgreen", inherit.aes = FALSE) +
  annotate("text", x = 195, y = 32, label = "alpha[3]", parse = TRUE, color = "darkgreen") +
